...
import (
	...
	"github.com/southwestmogrown/pokedexcli/internal"
)
...
// requestCache stores raw HTTP responses keyed by URL. An expiry
// interval is arbitrary; tests may override or reset the cache to avoid
// interference.
var requestCache = internal.NewPokeCache(1 * time.Minute)

// resetRequestCache clears the global cache (intended for tests).
func resetRequestCache() {
    requestCache = internal.NewPokeCache(1 * time.Minute)
}
...
func Map() error {
    ...
    if currentURL != "" && currentURL == lastFetchedURL {
        if currentLocations.Next != "" {
            ...
            // we were at the final page earlier and there was no next link;
            // clear the URL and return early (we already logged wrap above).
            currentURL = ""
            return nil
        }
    }

    // determine if we'll hit the cache; we don't want fetchURL to expose
    // that detail, so query the cache directly first.
    cacheHit := false
